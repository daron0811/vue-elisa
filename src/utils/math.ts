...
export function sd(arr: number[]): number {
    if (arr.length <= 1) return 0; const m = mean(arr); const v = mean(arr.map(x => (x - m) ** 2));
    return Math.sqrt(v);
}
...
export function inv4PL(y: number, p: { a: number, b: number, c: number, d: number }) {
    const { a, b, c, d } = p; const num = (a - d) / Math.max(y - d, 1e-12) - 1; if (num <= 0) return NaN;
    return c * Math.pow(num, 1 / b);
}
export function inv5PL(y: number, p: { a: number, b: number, c: number, d: number, e: number }) {
    const { a, b, c, d, e } = p; const num = Math.pow((a - d) / Math.max(y - d, 1e-12), 1 / e) - 1; if (num <= 0) return NaN;
    return c * Math.pow(num, 1 / b);
}
...
export function invertY(
    model: Model,
    params: any, // Linear:{m,b} / 4PL:{a,b,c,d} / 5PL:{a,b,c,d,e}
    y: number
): number {
    if (!Number.isFinite(y)) return NaN;
    if (model === 'Linear') {
        const m = params.m ?? 0, b = params.b ?? params.b_lin ?? 0;
        if (Math.abs(m) < 1e-12) return NaN;
        return (y - b) / m;
    }
    if (model === '4PL') return inv4PL(y, params);
