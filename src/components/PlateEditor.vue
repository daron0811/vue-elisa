            ...
            <table class="grid" @mouseup="onMouseUp">
                ...
                <tbody>
                    <tr v-for="(row, r) in plate" :key="r">
                        ...
                        <td v-for="(cell, c) in row" :key="c" :class="cellClass(cell, r, c)"
                            @mousedown.prevent="onDown(r, c)" @mouseenter="onEnter(r, c)">
                            <span class="tag" v-if="cell.type && cell.type !== 'BL'">{{ cell.type }}{{ cell.idx
                            }}</span>
...
.btn.is-active {
    ...
    box-shadow: 0 0 0 2px #8b8b8b inset;
}
