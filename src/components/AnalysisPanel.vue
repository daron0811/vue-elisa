<template>
  <section class="panel">
    <header class="panel__hdr"><h2>Step 4 — 擬合與繪圖</h2></header>
...
async function onAnalyze() {
  ...
  try {
    await store.analyze();
    store.computeBackcalc(); // 擬合完成後立即反算
  } catch (e: any) {
...
.row { display: grid; grid-auto-flow: column; grid-auto-columns: max-content; align-items: center; gap: .75rem; }
.ipt { padding: .4rem .6rem; border: 1px solid #bbb; border-radius: .4rem; min-width: 10rem; }
...
.err { color: #b00020; }
.grid { display: grid; grid-template-columns: 1fr 1.4fr; gap: 1rem; align-items: start; }
</style>
