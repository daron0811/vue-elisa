<template>
  <section class="panel">
    ...
    <div class="panel__row gradient">
      <div class="grp">
        ...
        <div class="grid">
          ...
          <button class="btn" @click="onAutofill" :disabled="!canAutofill">套用到 S1…S{{ indices.length }}</button>
        </div>
    ...
    <div class="panel__row list" v-if="indices.length">
      <table class="tbl">
        <thead><tr><th>標準品</th><th>濃度（{{ unit || 'ng/mL' }}）</th></tr></thead>
        <tbody>
          <tr v-for="idx in indices" :key="idx">
            <td>S{{ idx }}</td>
            <td><input class="ipt" type="number" step="0.001" min="0" :value="stdVal(idx)" @input="onStdInput(idx, $event)"/></td>
          </tr>
        </tbody>
      </table>
    </div>
...
.panel__row { display: grid; gap: .5rem; }
.lbl { color: #555; }
.ipt { padding: .45rem .6rem; border: 1px solid #bbb; border-radius: .4rem; min-width: 10rem; }
.btn { border: 1px solid #444; background: #111; color: #fff; padding: .45rem .8rem; border-radius: .6rem; cursor: pointer; }
.hint { font-size: .9rem; color: #666; }
.warn { color: #9a6700; }

.gradient .grid { display: grid; grid-template-columns: auto 12rem auto 12rem auto; gap: .5rem .75rem; align-items: center; }
